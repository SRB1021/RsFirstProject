// Draws the entire game onto the HTML canvas each time we get a new game state.
...
function drawMaze(ctx, dots) {
  ...
  for (let row = 0; row < dots.length; row++) {
    for (let col = 0; col < dots[row].length; col++) {
      ...
      if (cell === TILE_WALL) {
        // Draw a dark blue wall
        ctx.fillStyle = '#1a1aff';
        ctx.fillRect(x, y, TILE_SIZE, TILE_SIZE);
        // Slightly lighter border for a 3D look
        ctx.strokeStyle = '#3333ff';
        ctx.lineWidth = 1;
        ctx.strokeRect(x + 0.5, y + 0.5, TILE_SIZE - 1, TILE_SIZE - 1);

      } else if (cell === TILE_DOT) {
        // Small white dot in the center of the tile
...
function drawGhosts(ctx, ghosts, myGhostName) {
  ...
  for (const [name, ghost] of Object.entries(ghosts)) {
    ...
    // White ring around YOUR ghost so you know which one you are
    if (name === myGhostName) {
      ctx.strokeStyle = '#fff';
      ctx.lineWidth = 2;
      ctx.beginPath();
